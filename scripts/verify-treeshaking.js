...
function checkTreeShaking() {
  ...
  if (jsFiles.length === 0) {
    console.log('❌ No JavaScript files found. Please run `npm run build` first.');
    return;
  }

  // Icons we intentionally import
  const usedIcons = [
    'Calendar', 'Users', 'Vote', 'FileText', 'Settings', 
    'ChevronRight', 'Heart', 'Star', 'Download', 'Share', 'PlusCircle'
  ];
  
  // Common icons that should NOT be in bundle if tree-shaking works
  const unusedIcons = [
    'Airplane', 'Anchor', 'Archive', 'Bell', 'Camera', 'Cloud', 
    'Database', 'Globe', 'Home', 'Lock', 'Mail', 'Map', 'Phone'
  ];
  ...
  if (foundUnusedIcons.length > 0) {
    ...
    console.log('   This might indicate tree-shaking is not working optimally.');
  } else {
  ...
  if (foundUnusedIcons.length === 0) {
    console.log('   ✅ PASSED - Only imported icons are included in the bundle');
  } else {
    console.log('   ❌ PARTIAL - Some unused icons were found in the bundle');
  }

  console.log('\n📋 Recommendations:');
  console.log('   - Only import specific icons you need: import { IconName } from "lucide-react"');
  console.log('   - Avoid import * as Icons from "lucide-react"');
  console.log('   - ShadCN UI components use class-variance-authority for optimal bundling');
  console.log('   - CSS custom properties allow theme switching without JS bundle bloat');
}
